...
export default async function handler(req, res) {
  ...
  try {
    ...
    const count = Math.min(Math.max(parseInt(fields.count?.[0] || fields.count || '9', 10), 1), 12);
    const uploadedText = await extractText(uploaded);

    const systemPrompt = [
      `당신은 인스타그램 콘텐츠 전략가입니다. 주어진 스토리보드를 분석하여 인스타그램 그리드에 어울리는 ${count}개의 피드 게시물을 기획해주세요. 전체 그리드가 통일된 색감, 분위기, 스타일을 유지하도록 구성하세요.`,
      promptTemplate ? `\n\n## 항상 적용할 기준\n${promptTemplate}` : ''
    ].join('');
